...
def test_ct_range_manual(image_rand_pos_gpu: Union[torch.Tensor, np.ndarray]) -> None:
...
def test_ct_range_liver(image_rand_pos_gpu: Union[torch.Tensor, np.ndarray]) -> None:
    image_out = CTRange.transform(
        data=image_rand_pos_gpu,
        output_range=output,
        window=200,
