...
def test_create_lr_scheduler_last_epoch() -> None:
    ...
    l_rate = 1e-3
    gamma = 0.5
    expected_lrs_per_epoch = [l_rate, l_rate * gamma, l_rate * (gamma ** 2)]
    config = DummyModel()
    ...
    assert np.allclose(enumerate_scheduler(lr_scheduler, 3), expected_lrs_per_epoch)
    # create lr scheduler for recovery checkpoint
    config.start_epoch = 1
    lr_scheduler2, _ = _create_lr_scheduler_and_optimizer(config)
    assert lr_scheduler2.last_epoch == config.start_epoch
    # check lr scheduler initialization matches the checkpoint epoch
    # as training will start for start_epoch + 1 in this case
    assert np.allclose(enumerate_scheduler(lr_scheduler2, 2), expected_lrs_per_epoch[1:])
...
def test_warmup_against_original_schedule(lr_scheduler_type: LRSchedulerType, warmup_epochs: int) -> None:
    ...
    if ((lr_scheduler_type == LRSchedulerType.Polynomial or
         lr_scheduler_type == lr_scheduler_type.LRSchedulerType.Cosine)
            and warmup_epochs > 0):
...
def test_resume_from_saved_state(lr_scheduler_type: LRSchedulerType, warmup_epochs: int) -> None:
    ...
    # Scheduler 2 mimics what happens if we train for the full set of epochs
    expected_lr_list = enumerate_scheduler(lr_scheduler_2, config.num_epochs)

    # Scheduler 1 is only run for 4 epochs, and then "restarted" to train the rest of the epochs.
    result_lr_list = enumerate_scheduler(lr_scheduler_1, restart_from_epoch)
    ...
    # Load a "checkpoint"
    lr_scheduler_resume.load_state_dict(lr_scheduler_1.state_dict())
    result_lr_list.extend(enumerate_scheduler(lr_scheduler_resume, config.num_epochs - restart_from_epoch))
