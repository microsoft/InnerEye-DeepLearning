...
EXTERNAL_CHECKPOINT_URL_EXAMPLE = "https://download.pytorch.org/models/resnet18-5c106cde.pth"
...
def test_use_local_weights_file(test_output_dirs: OutputFolderForTests) -> None:
    ...
    config.checkpoint_urls = EXTERNAL_CHECKPOINT_URL_EXAMPLE
    checkpoint_handler.download_recovery_checkpoints_or_inference_checkpoints()
...
def test_get_all_checkpoints_from_ensemble_run(test_output_dirs: OutputFolderForTests) -> None:
    ...
    checkpoint_paths = manage_recovery.get_best_checkpoints()
    assert checkpoint_paths
...
def test_get_checkpoints_to_test(test_output_dirs: OutputFolderForTests) -> None:
    ...
    create_checkpoint_file(local_weights_path)
    config.local_checkpoint_paths = local_weights_path
    manage_recovery.download_recovery_checkpoints_or_inference_checkpoints()
...
def test_download_model_weights(test_output_dirs: OutputFolderForTests) -> None:
    ...
    model_config = DummyModel(weights_url=EXTERNAL_CHECKPOINT_URL_EXAMPLE)
...
def test_get_local_checkpoint_paths_or_download(test_output_dirs: OutputFolderForTests) -> None:
    config = ModelConfigBase(should_validate=False)
    manage_recovery = get_default_checkpoint_handler(model_config=config,
                                                     project_root=test_output_dirs.root_dir)
    ...
    with pytest.raises(ValueError) as ex:
        manage_recovery.get_local_checkpoint_paths_or_download()
        assert "neither local_weights_path nor weights_url is set in the model config" in ex.value.args[0]

    # If local_weights_path folder exists, get_local_checkpoint_paths_or_download should not do anything.
    local_weights_path = manage_recovery.project_root / "exist.pth"
    create_checkpoint_file(local_weights_path)
    manage_recovery.model_config.local_checkpoint_paths = local_weights_path
    returned_weights_path = manage_recovery.get_local_checkpoint_paths_or_download()
    assert local_weights_path == returned_weights_path

    # Pointing the model to a URL should trigger a download
    config.local_checkpoint_paths = None
    config.checkpoint_urls = EXTERNAL_CHECKPOINT_URL_EXAMPLE
    downloaded_weights = manage_recovery.get_local_checkpoint_paths_or_download()
    ...
    expected_path = manage_recovery.project_root / MODEL_WEIGHTS_DIR_NAME / \
                    os.path.basename(urlparse(EXTERNAL_CHECKPOINT_URL_EXAMPLE).path)
    ...
    downloaded_weights_new = manage_recovery.get_local_checkpoint_paths_or_download()
