...
config.dataset.name = 'CIFAR10'  # Dataset name. Available choices: "CIFAR10", "RSNAKaggle"
config.dataset.dataset_dir = ''  # Local Path to dataset directory. Automatically set for CIFAR10.
config.dataset.num_workers = None  # Number of cpu workers to use in dataloaders.

config.train = ConfigNode()
config.train.seed = None  # Training seed.
config.train.batch_size = None  # Batch size for both training and validation
config.train.output_dir = None  # Runs will be saved to repo_root / SSL_EXPERIMENT_DIR / output_dir.
config.train.resume_from_last_checkpoint = False  # Whether to continue training from the last checkpoint of a run.
# The checkpoint is assumed to be already placed repo_root / SSL_EXPERIMENT_DIR / output_dir and named last_checkpoint
config.train.base_lr = None  # Starting learning rate
config.train.self_supervision = ConfigNode()
config.train.self_supervision.type = None  # Which algorithm to use for SSL training. Choices: "byol", "sim_clr".
config.train.self_supervision.encoder_name = None  # Encoder to use: resnet18, resnet50, resnet101 or densenet121.
config.train.self_supervision.use_balanced_binary_loss_for_linear_head = False  # Whether to use balanced binary
# cross-entropy loss for linear head training. Weights are calculated based on statistics of the training set.
config.train.checkpoint_period = 200  # At which frequency to save checkpoints.

config.scheduler = ConfigNode()
config.scheduler.epochs = None  # Total number of epochs to train for.

# The parameters below are used to specify which preprocssing and augmentations to use and their strength for
# RSNAKaggle dataset.
...
# Whether to apply random crop of scale config.augmentation.random_crop.scale, final crop is then resized to
# config.preprocess.resize cf. torchvision RandomCropResize function. If use_random_crop set to False,
# only apply resize.
config.augmentation.use_random_crop = False
# Whether to apply random horizontal flip applied with probability config.augmentation.random_horizontal_flip.prob.
...
# Whether to apply random color brightness and contrast transformation parametrized by random_color.brightness
# and random_color.contrast.
config.augmentation.use_random_color = False
# Whether to add gaussian noise with probability gaussian_noise.p_apply and standardized deviation gaussian_noise.std.
...
# Whether to apply CutOut of a portion of the image. Size of cutout is parametrized by random_erasing.scale,
# tuple setting the minimum and maximum percentage of the image to remove and by random_erasing.ratio setting
# the min and max ratio of the erased patch.
config.augmentation.use_random_erasing = False
# Whether to apply elastic transforms. If True, the transformation is applied with probability
# elastic_transform.p_apply.And is parametrized by elastic_transform.sigma and elastic_transform.alpha.
