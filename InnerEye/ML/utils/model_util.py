...
class ModelAndInfo:
    ...
    def _load_checkpoint(cls, model: DeviceAwareModule, checkpoint_path: Path,
                         key_in_state_dict: str, use_gpu: bool, use_distributed_data_parallel: bool) -> int:
        """
        ...
        if use_distributed_data_parallel:
            ...
            result = model.module.load_state_dict(checkpoint[key_in_state_dict], strict=False)
        else:
...
def generate_and_print_model_summary(config: ModelConfigBase, model: DeviceAwareModule, device: torch.device) -> None:
    """
