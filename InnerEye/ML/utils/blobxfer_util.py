...
def upload_blobs(account: str, account_key: str, blobs_destination_path: str, source: Path,
                 config: Optional[BlobXFerConfig] = None) -> None:
    """
    Upload a given set of files to Azure blob storage from local source path, via blobxfer.

    :param account: The name of the storage account to access the files.
    :param account_key: The key for the storage account.
    :param blobs_destination_path: The path of the files that should be uploaded to. This must be in format
    'container/file_prefix/', ending with a slash (will be added if not provided).
    :param source: The source folder for files to be uploaded from the local machine.
    :param config: BlobXFerConfig to use for upload configuration, use default presets if None.
    """
    if not config:
        config = BlobXFerConfig.create_default(account=account, account_key=account_key)

    if not blobs_destination_path.endswith("/"):
        blobs_destination_path += "/"

    start_time = time.time()
    # the account name can be an Azure Resource ID so extract the name from it if this is the case
    logging.info(f"Uploading '{source}' to {blobs_destination_path} on storage account {config.account_name}")

    blobs_root_path = to_azure_friendly_container_path(Path(blobs_destination_path))
    _check_blobs_path(blobs_root_path)

    upload = config.get_upload_options()
    local_path = blobxfer.api.LocalSourcePath()
    local_path.add_path(str(source))
    # noinspection PyTypeChecker
    upload_spec = blobxfer.api.UploadSpecification(upload, config.skipon_options, local_path)
    dst = blobxfer.api.AzureDestinationPath()
    dst.add_path_with_storage_account(blobs_root_path, config.account_name)
    # noinspection PyTypeChecker
    upload_spec.add_azure_destination_path(dst)
    # noinspection PyTypeChecker
    uploader = blobxfer.api.Uploader(config.general, config.get_credentials(), upload_spec)
    uploader.start()
    elapsed = time.time() - start_time
    logging.info(f"Finished uploading in {elapsed:0.2f}sec.")


def _check_blobs_path(blobs_path: str) -> None:
    blobs_root_path_dirs = blobs_path.rstrip("/").split("/")
    if len(blobs_root_path_dirs) < 2:
        raise ValueError(f"blobs_path must be in format 'container/blob prefix', found: {blobs_path}")
