...
def plot_scores_for_csv(path_csv: str) -> None:
    ...
    with pd.option_context('display.max_rows', None, 'display.max_columns', None, 'display.width', 150):
        ...
        display_metric(df, MetricsFileColumns.HausdorffDistanceMM.value, high_values_are_good=False)
        display_metric(df, MetricsFileColumns.MeanDistanceMM.value, high_values_are_good=False)


def display_metric(df: pd.DataFrame, metric_name: str, high_values_are_good: bool) -> None:
    """
...
def worst_patients(df: pd.DataFrame, metric_name: str, ascending: bool) -> str:
    """
