...
def UncertaintyMapFeatures(self,subjects):
    '''extract PyRadiomics features from the uncertainty map'''

    for i,subject in enumerate(subjects):
        subject_dir = self.outputs_folder / f"{int(patient_id):03d}"
...
def UncertaintyAggregate(config):
    ...
    df.dice = average_dice_across_structures(self,subjects,metrics_df)
    features = UncertaintyMapFeatures(self, features, subject_id, i)
        #dice.append(average_dice_across_structures())

    features = self.PostProcessFeatures(features)
