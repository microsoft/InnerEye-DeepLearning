" This module has been copied from https://github.com/Project-MONAI/tutorials/tree/master/pathology/multiple_instance_learning"
import os
import time
import shutil
import argparse
        ...
        for idx, batch_data in enumerate(loader):
            ...
            with autocast(enabled=args.amp):
                ...
                if max_tiles is not None and data.shape[1] > max_tiles:
                    ...
                    if has_extra_outputs:
                        extra_outputs["layer1"] = torch.cat([logit[0] for logit in logits2], dim=0)
                        extra_outputs["layer2"] = torch.cat([logit[1] for logit in logits2], dim=0)
                        extra_outputs["layer3"] = torch.cat([logit[2] for logit in logits2], dim=0)
                        extra_outputs["layer4"] = torch.cat([logit[3] for logit in logits2], dim=0)
