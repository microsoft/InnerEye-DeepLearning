...
class CosineScheduler:
    ...
    def __call__(self, step):
        return self.base_lr * (
                self.lr_min_factor + (1 - self.lr_min_factor) * 0.5 * (1 + np.cos(step / self.steps * np.pi)))
...
class ExponentialScheduler:
    ...
    def __call__(self, step):
        return self.base_lr * (self.lr_start_factor + (1 - self.lr_start_factor) * (step / self.steps) ** self.exponent)
...
class LinearScheduler:
    ...
    def __call__(self, step):
        return self.lr_start + (self.lr_end - self.lr_start) * step / self.steps
