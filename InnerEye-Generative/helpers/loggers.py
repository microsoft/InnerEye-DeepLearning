...
class AzureMLLogger(LightningLoggerBase):
    ...
    @rank_zero_only
    def log_image(self, key: str, value: torch.Tensor, step: Optional[int] = None) -> None:
        if self.is_azureml_run:
            assert(len(value.shape), 3)
            value = np.moveaxis(value.detach().cpu().numpy(),0, -1)
            fig, ax = plt.subplots(figsize=(20, 20))
            ax.imshow(value)
            plt.title(step if step is not None else '')
            RUN_CONTEXT.log_image(key, plot=plt)
...
class TensorboardWithImgs(TensorBoardLogger):
    def __init__(self,
        save_dir: str,
        ...
        prefix: str = '',
        **kwargs):
        super().__init__(save_dir, name, version, log_graph, default_hp_metric, prefix, **kwargs)

    @rank_zero_only
    def log_image(self, key: str, val: torch.Tensor, step: Optional[int]=None) -> None:
        val = val.detach().cpu().numpy()
        self.experiment.add_image(key, val, step)

class LoggerCollectionWithImgs(LoggerCollection):
    def __init__(self, logger_iterable: Iterable[LightningLoggerBase]):
        super().__init__(logger_iterable)

    def log_image(self, key: str, val: torch.Tensor, step: Optional[int]=None) -> None:
        for logger in self._logger_iterable:
            logger.log_image(key, val, step)
