...
```conda env create --file <environment.yml or environment_win.yml>```

Select the relevant file depending on your OS platform (`environment.yml` for Linux, `environment_win.yml` for windows). This will create a new conda environment which can be activated by running:
