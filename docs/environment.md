...
To create an environment from one of the lockfiles, run the following command:

```conda env create --file <environment.yml/environment_win.yml>```

Selecting the relevant file depending on your OS platform (`environment.yml` for Linux, `environment_win.yml` for windows). This will create a new conda environment which can be activated by running:
...
NOTE: any changes to packages made here will not be repeated in any AML deployments. To edit package versions to be used in AML (i.e. to generate a new lockfile `environment.yml`) you will need to follow the steps on creating an conda env in Linux (previous section). If you do not have access to a Linux machine, we highly recommened using [WSL](#set-up-windows-subsystem-for-linux).
...
In order to create the Conda environment you will need to have build tools installed on your machine. If you are running Windows, they should be already installed with Conda distribution.
